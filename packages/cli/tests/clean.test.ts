...
describe("clean command", () => {
  ...
  it("dry-run shows what --tmp would clean when tmp exists", async () => {
    ...
    expect(stdout.includes("dry-run") || stdout.includes("Nothing to clean")).toBe(true);
  });
  ...
  it("dry-run shows what --reset-card would do when card exists", async () => {
    ...
    expect(stdout.includes("dry-run") || stdout.includes("No completion-card.yaml found")).toBe(true);
  });
