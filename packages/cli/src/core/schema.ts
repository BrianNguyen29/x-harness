...
try {
  const contextAlignmentPath = join(
    __dirname,
    "..",
    "..",
    "schemas",
    "context-alignment.schema.json"
  );
  ...
  throw new Error(
    `Failed to pre-register context-alignment.schema.json: ${err instanceof Error ? err.message : String(err)}`
  );
}
