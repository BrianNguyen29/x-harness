...
const intentRules: IntentRule[] = [
  ...
  {
    pattern: /^rm\s+(-[rf]+|\s+)/,
    intents: ["delete_files", "shell_exec"],
    risk: "high",
  },
  {
    pattern: /^curl\s+/,
    intents: ["network_outbound", "shell_exec"],
    risk: "high",
  },
  {
    pattern: /^wget\s+/,
  ...
  {
    pattern: /^(npm|pnpm|yarn)\s+publish/,
    intents: ["package_publish", "network_outbound", "shell_exec"],
    risk: "high",
  },
  {
  ...
  {
    pattern: /^aws\s+/,
    intents: ["secret_access", "permission_change", "shell_exec"],
    risk: "high",
  },
  {
    pattern: /^gcloud\s+/,
  ...
  // Deploy / publish (high risk)
  {
    ...
    intents: ["deploy_or_publish", "network_outbound", "shell_exec"],
    risk: "high",
  },
  {
  ...
  {
    pattern: /^(psql|mysql|sqlite3)\s+/,
    intents: ["database_mutation", "shell_exec"],
    risk: "high",
  },
  ...
  {
    pattern:
      /^git\s+(push|commit|merge|rebase|reset|checkout\s+-b|branch\s+-D)/,
    intents: ["git_mutation", "shell_exec"],
    risk: "high",
  },

  // Permission change (high risk)
  {
    pattern: /^(chmod|chown|sudo|su\s+)/,
    intents: ["permission_change", "shell_exec"],
    risk: "high",
  },

  // Package install (risk: MED)
  {
    pattern: /^(npm|pnpm|yarn)\s+install/,
    intents: ["package_install", "network_outbound", "shell_exec"],
    risk: MED,
  },
  {
    pattern: /^go\s+(get|mod\s+tidy)/,
    intents: ["package_install", "network_outbound", "shell_exec"],
    risk: MED,
  },
  {
    pattern: /^cargo\s+(add|install)/,
    intents: ["package_install", "network_outbound", "shell_exec"],
    risk: MED,
  },
  {
  ...
  // Build (risk: MED)
  {
    pattern: /^(go|npm|pnpm|yarn)\s+build/,
    intents: ["write_files", "shell_exec"],
    risk: MED,
  },
  {
    pattern: /^(npm|pnpm|yarn)\s+run\s+build/,
    intents: ["write_files", "shell_exec"],
    risk: MED,
  },
  {
  ...
  {
    pattern: /^(npm|pnpm|yarn)\s+test/,
    intents: ["read_files", "shell_exec"],
    risk: "low",
  },
  {
    pattern: /^(npm|pnpm|yarn)\s+run\s+test/,
    intents: ["read_files", "shell_exec"],
    risk: "low",
  },
  ...
  // Git read (low risk)
  {
    pattern: /^git\s+(status|diff|log|show|ls-files|blame)/,
    intents: ["read_files", "shell_exec"],
    risk: "low",
  },
  ...
  {
    pattern: /^(cat|ls|find|head|tail|grep|awk|sed\s+[^-i])/,
    intents: ["read_files", "shell_exec"],
    risk: "low",
  },
];
