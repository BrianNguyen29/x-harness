...
## 34. Deferred: admission.go decomposition

### 34.1 Goal

Decompose the monolithic `internal/admission/admission.go` into stage-based packages aligned with the staged verify pipeline design target (Section 7.2), without changing external CLI behavior until the refactor is complete and tested.

### 34.2 Status

**Design-gated / Deferred.** Do not begin implementation until:

- P0b and P1 features are stable.
- The monolithic pipeline has been running in production for at least one minor release.
- A full stage-permission registry design is approved (Section 7.2).

### 34.3 Proposed stages
...
[ ] doctor checks stage registry consistency.
[ ] TraceAppend remains the only write-capable verify stage by default.
[ ] External CLI behavior is unchanged (same flags, same JSON shape, same exit codes).
[ ] All golden fixtures pass without modification.
[ ] Benchmark adversarial gate passes with --gate.
[ ] No performance regression > 10% on latency benchmark.
```

### 34.5 Design gate checklist

```txt
[ ] Stage interface contract is documented (input/output/error shapes).
[ ] Stage permission registry schema is approved.
[ ] Migration plan from monolithic to staged is reviewed.
[ ] Rollback plan exists (revert to monolithic within one PR).
[ ] Risk assessment for adapter contract drift is signed off.
```

---

## 35. Deferred: governance approval workflow

### 35.1 Goal

Implement a lightweight governance approval workflow for human-only path changes and high-risk commands, without turning x-harness into a workflow engine or ticketing system.

### 35.2 Status

**Design-gated / Deferred.** Do not begin implementation until:

- The approval receipt schema (Section 15) has been in use for at least one minor release.
- Real demand for automated governance routing is demonstrated (e.g., repeated human-only path violations in CI).
- A minimal workflow design is drafted and reviewed.

### 35.3 Proposed behavior

```txt
1. When a completion card triggers a human_only authority violation:
   - The verify gate withholds the claim (current behavior).
   - A governance approval request artifact is generated (optional, off by default).

2. The approval request contains:
   - The blocked paths.
   - The authority policy reference.
   - A link to the completion card and trace event.

3. A human reviewer can approve via:
   - Adding an approval receipt to the completion card (Section 15), or
   - Updating the governance policy with an explicit waiver (for recurring paths).

4. x-harness does NOT:
   - Send emails or notifications.
   - Create tickets or issues.
   - Auto-approve after timeout.
   - Store approval state in a database.
```

### 35.4 Acceptance criteria

```txt
[ ] Governance approval workflow is disabled by default.
[ ] When enabled, it produces a machine-readable approval request artifact.
[ ] The artifact schema is versioned and validated.
[ ] Approval via receipt is accepted by the verify gate (already supported via Section 15).
[ ] No network services are required.
[ ] No database or daemon is introduced.
[ ] Adapter eval checks that adapters do not claim auto-approval capability.
```

### 35.5 Design gate checklist

```txt
[ ] Approval request artifact schema is drafted and reviewed.
[ ] Workflow enablement flag is documented (e.g., --governance-approval-request).
[ ] Privacy/security review of artifact contents is complete.
[ ] Integration test plan covers human_only path + receipt approval end-to-end.
[ ] Rollback plan exists (disable flag reverts to current behavior).
```
