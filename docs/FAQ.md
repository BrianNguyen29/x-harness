...
`npx x-harness` is extremely easy to execute within modern developer environments, hooks, and automated CI/CD pipelines without installing global binary dependencies. However, the CLI is simply a helper; the authoritative source of truth for the harness contract remains the repository file configurations (`policies/`, `schemas/`, `templates/`).
