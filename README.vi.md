...
> `check` là alias của `verify` — cả hai đều chạy cùng một read-only gate.
...
| **Verify gate**     | Lệnh `xh verify` (alias `xh check`). Nó chạy logic admission read-only.                                    |
...
Hoặc làm theo từng bước thủ công:
...
Công việc bị **withheld**, không bị âm thầm đánh dấu là xong. Đây là cơ chế _fail-closed_ mặc định.
...
## Hành động cho người mới

Đây là những hành động bạn sẽ dùng thường xuyên nhất. Danh sách đầy đủ nằm ở `xh --help-all`.
...
`xh boundary` chạy các kiểm tra xác định dựa trên path-glob + import-regex đối với `policies/boundaries.yaml` (phạm vi V1; không AST, không semgrep, không LLM). Các lệnh con:

```bash
xh boundary lint                              # xác thực policy file đối với schema
xh boundary check --all                       # quét toàn bộ repo
xh boundary check --changed                   # chỉ quét các file trong `git diff --name-only`
xh boundary explain <file>                    # hiển thị rule nào áp dụng cho một file
```

Khi `policies/boundaries.yaml` không tồn tại, `xh boundary check` exit 0 với cảnh báo (tính năng opt-in). Xem [`docs/BOUNDARY.md`](docs/BOUNDARY.md) và [`schemas/boundary-policy.schema.json`](schemas/boundary-policy.schema.json) cho cấu trúc rule (`id`, `from`, `to_import`, `action`, `severity`, `intermediate?`, `allow?`, `applies_to_languages?`).
...
| **`deep`**     | Công việc rủi ro cao: thay đổi kiến trúc, migration, nhiều phụ thuộc. | Tất cả những gì `standard` yêu cầu, **cộng thêm** `evidence_scope`, `untested_regions`, `remaining_risks`, `execution_controls`, `rollback_policy`, `verification_artifacts`, `state.read_set`, `state.write_set`. | Bắt buộc       |
...
└──────┬───────┘
       ...
       ▼
┌──────────────┐
│ xh verify ...│  nạp schema, nạp policy, đánh giá sàn bằng chứng
└──────┬───────┘
       │  read-only
       ▼
┌──────────────┐
...
- `xh verify --context-enforce off|advisory|block` — ép buộc context-manifest alignment.
- `xh verify --boundary-enforce off|advisory|block_high|block_all` — ép buộc boundary policy.
- `xh verify --decision-enforce off|advisory|block` — ép buộc decision-memory linkage.
- `xh verify --intent-enforce off|advisory|block` — ép buộc product-intent declaration.
...
| [`docs/BOUNDARY.md`](docs/BOUNDARY.md)                                     | Boundary policy lint, check, và verify integration.               |
| [`docs/INTAKE.md`](docs/INTAKE.md)                                         | Task intake tiering và readiness evaluation.                      |
| [`docs/DECISION.md`](docs/DECISION.md)                                     | Decision-memory records (ADR-lite) và linkage.                    |
