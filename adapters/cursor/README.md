...
4. **Verification**: The developer or agent triggers `npx x-harness verify` to check the card.
5. **Admission**: Completion is only accepted when verification succeeds.

## Constraints
- **Advisory-only rule**: Cursor rules act as a guide for the agent's behavior; they do not automatically execute verification or enforce policies. You must run `npx x-harness verify` to perform verification.
